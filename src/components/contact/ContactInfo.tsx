  ...
  return (
    <div className="flex flex-col gap-4">
      {contactDetails.map((item, index) => {
        ...
        if (item.title === "Phone Number" && phone) {
          displayContent = phone.display;
          href = `tel:${phone.raw}`;
        } else if (item.title === "Email Address" && email) {
          displayContent = email.display;
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              ...
              {href ? (
                <a
                  href={href}
                  className={`block text-sm sm:text-base font-bold text-gray-900 group-hover:text-primary transition-colors ${item.title === "Email Address" ? "break-all" : "wrap-break-word"}`}
                >
                  {displayContent}
                </a>
              ) : (
